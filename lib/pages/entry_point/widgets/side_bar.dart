import 'package:animately/models/menu.dart';
import 'package:animately/models/menus.dart';
import 'package:animately/pages/entry_point/widgets/info_card.dart';
import 'package:animately/pages/entry_point/widgets/side_menu.dart';
import 'package:animately/utils/rive_utils.dart';
import 'package:flutter/material.dart';
import 'package:rive/rive.dart';

class SideBar extends StatefulWidget {
  const SideBar({super.key});

  @override
  State<SideBar> createState() => _SideBarState();
}

class _SideBarState extends State<SideBar> {
  Menu selectedSideMenu = sidebarMenus.first;
  @override
  Widget build(BuildContext context) {
    return SafeArea(
      child: Container(
        height: double.infinity,
        width: 288.0,
        decoration: const BoxDecoration(
          color: Color(0xFF17203A),
          borderRadius: BorderRadius.all(
            Radius.circular(30.0),
          ),
        ),
        child: DefaultTextStyle(
          style: const TextStyle(color: Colors.white),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const InfoCard(name: 'Amine', bio: 'A group of people'),
              Padding(
                padding: const EdgeInsets.only(left: 24, top: 32, bottom: 16),
                child: Text(
                  'Browse'.toUpperCase(),
                  style: Theme.of(context)
                      .textTheme
                      .titleMedium!
                      .copyWith(color: Colors.white70),
                ),
              ),
              ...sidebarMenus
                  .map(
                    (menu) => SideMenu(
                      menu: menu,
                      selectedMenu: menu,
                      press: () {
                        RiveUtils.chnageSMIBoolState(menu.rive.status!);
                        setState(() => selectedSideMenu = menu);
                      },
                      riveOnInit: (artboard) =>
                          menu.rive.setStatus = RiveUtils.getRiveInput(
                        artboard,
                        stateMachineName: menu.rive.stateMachineName,
                      ),
                    ),
                  )
                  .toList(growable: false),
              Padding(
                padding: const EdgeInsets.only(left: 24, top: 40, bottom: 16),
                child: Text(
                  "History".toUpperCase(),
                  style: Theme.of(context)
                      .textTheme
                      .titleMedium!
                      .copyWith(color: Colors.white70),
                ),
              ),
              ...sidebarMenus2
                  .map(
                    (menu) => SideMenu(
                      menu: menu,
                      selectedMenu: menu,
                      press: () {
                        RiveUtils.chnageSMIBoolState(menu.rive.status!);
                        setState(() => selectedSideMenu = menu);
                      },
                      riveOnInit: (artboard) =>
                          menu.rive.setStatus = RiveUtils.getRiveInput(
                        artboard,
                        stateMachineName: menu.rive.stateMachineName,
                      ),
                    ),
                  )
                  .toList(growable: false),
            ],
          ),
        ),
      ),
    );
  }
}
